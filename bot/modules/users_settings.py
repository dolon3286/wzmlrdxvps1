...
<b>High-resolution YouTube videos:</b> The bot uses yt-dlp's <code>tv</code> player client to avoid the low-resolution <code>tv_downgraded</code> fallback. If YouTube offers 4K or 8K for a video, it will appear in the quality selector (including 4320p for playlists). A valid PO Token may still be required for restricted streams; configure it with <code>"extractor_args": {"youtube": {"po_token": ["web.gvs+TOKEN"]}}</code> and follow the <a href='https://github.com/yt-dlp/yt-dlp/wiki/PO-Token-Guide'>yt-dlp PO Token guide</a>. Keep tokens private.
